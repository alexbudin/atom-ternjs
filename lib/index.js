...
module.exports = {
    configDefaults: {
        ...
        expandHintsForward: false,
        caseInsensitiveHints: false,
        guessWhenNoDetailsFound: true,
    ...
    customGetSugestionsAdded: false,
    activate: function(state) {
        ...
        self.editorSubscription = atom.workspaceView.eachEditorView(function(editor) {
            if (editor.attached && !editor.mini) {
                ...
                var definitionsView = new DefinitionsView(editor);

                editor.on('editor:will-be-removed', function() {
                    if (!autocompleteView.hasParent()) {
                        autocompleteView.remove();
                    }
                    if(!definitionsView.hasParent()) {
                        definitionsView.remove();
                    }
                    var index = self.autocompleteViews.indexOf(autocompleteView);
                    if (index > -1) {
                        self.autocompleteViews.splice(index, 1);
                    }
                    index = self.definitionsViews.indexOf(definitionsView);
                    if (index > -1) {
                        self.definitionsViews.splice(index, 1);
                    }
                });
                self.autocompleteViews.push(autocompleteView);

                self.definitionsViews.push(definitionsView);

                if(!self.customGetSugestionsAdded) {
                    self.customGetSugestionsAdded = true;
                    return self.autocompleteViews[0].setCustomGetSugestions(requestHints);
                }
            }
        });

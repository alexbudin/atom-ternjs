        ...
        if (completions.length > 0 || !isProperty) {
            ...
            if (completions.length > 0) {
                ...
                for (var i = 0; i < completions.length; i++) {
                    comps.push({
                        word: completions[i].value,
                        suffix: '',//completions[i].value,
                        prefix: ''//completions[i].value
                    });
                }
                if(!autocompleteView.attached) {
                    autocompleteView.attach();
                }
                autocompleteView.setItems(comps);
                autocompleteView.populateList();
            } else {
                if(autocompleteView.attached) {
                    autocompleteView.cancelled();
                }
            }
